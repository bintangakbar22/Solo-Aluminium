...
import { connectionChecker, getTransactions, createTransaction, updateTransaction, deleteTransaction,rupiah, getProducts,FinanceScreen } from '../../../Redux/actions';
import { useIsFocused } from '@react-navigation/native';
import { COLORS,FONTS } from '../../../Utils';
import {Formik} from 'formik';
import * as yup from 'yup';
import { Input,BottomModal,Header,Button, ButtonBorder,CardBorder } from '../../../Components';
import DropDownPicker from 'react-native-dropdown-picker';
const FinanceProduct = () => {
  const isFocused = useIsFocused();
  const dispatch = useDispatch();
  const accessToken = useSelector(state => state.appData.accessToken);
  const transaction = useSelector(state => state.appData.transactions);
  console.log(transaction)
  const transactions = transaction.filter(i=>i.transtype=="product");
  ...
  return (
    <View style={styles.Container}> 
      <Header title={'Transaksi'} print input onPress={()=>{setopenModal(true);setLabel('input')}} finance  />
      <FlatList

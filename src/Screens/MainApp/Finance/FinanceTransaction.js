...
const FinanceTransaction = () => {
  ...
  const transactions = transaction.filter(i=>i.transtype=="finance");
  ...
  const renderItem = ({item}) => {
    ...
    return(
      <View style={{marginVertical:ms(5)}} >
        <ButtonBorder label={item.keterangan} 
          color={item.type=='debit'?COLORS.green:COLORS.red}
          value={`Rp. ${rupiah(item.total)}`} 
          onPress={()=>{
            setLabel('update');
            setItem(item)
            setValue(item.type)
  ...
  return (
    <View style={styles.Container}> 
      <Header title={'Transaksi'} input onPress={()=>{setopenModal(true);setLabel('input')}} finance/>
      <FlatList
        ...
        {openModal && (
          <BottomModal onDismiss={()=>{setopenModal(false)}}>
            <Formik
              initialValues={{
                total: label=='input'?'':item.total.toString(),
                keterangan : label=='input' ?'':item.keterangan
              }}
